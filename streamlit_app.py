...
# Custom CSS for improved styling (remains the same)
st.markdown("""
<style>
.main-header {
...
metrics = {
    ...
    'Decision Tree': {
        ...
        'recall': 0.2509,
        'f1': 0.0982,
        'auc': 0.574,  # Updated from the ROC curve data
        'confusion_matrix': [[10888, 1107], [215, 72]]
    },
    'Gradient Boosting': {
        'accuracy': 0.9761,
        'precision': 0.3846,
...
# Updated ROC curve data with Altman Z-score
roc_curves = {
    'Altman Z-score': {
...
pages = ["Overview", "Dataset Information", "Altman Z-Score Analysis", "Model Comparison", "ROC Curves", "Feature Importance", "Confusion Matrices"]
...
if selected_page == "Overview":
    ...
    This project uses the Kaggle American Companies Bankruptcy Prediction dataset (financial data from 1999-2018 for ~8,000 US public companies) to train machine learning models and apply the traditional Altman Z-Score method for bankruptcy prediction. Our app showcases the predictions and performance metrics, highlights key financial features, and allows users to explore what-if scenarios.
    """)
    ...
    - Altman Z-Score (Traditional Financial Analysis Method)
    - Decision Tree
    - Gradient Boosting
    - Random Forest
    - Logistic Regression
    ...
    **Note on Class Imbalance**: The dataset has a significant class imbalance with many more 'alive' companies than 'failed' ones. 
    ...
    # Add comparison with traditional Z-Score
    st.markdown("### Traditional vs. Machine Learning Methods")
    st.markdown("""
    The Altman Z-Score is a traditional financial analysis method that has been used since 1968. It offers several advantages:
    - Simple to calculate and interpret
    - Based on fundamental financial ratios
    - No need for complex machine learning infrastructure
    
    However, machine learning models can often provide better performance by:
    - Learning complex patterns in the data
    - Adapting to different types of companies
    - Incorporating more features than the Z-Score's five ratios
    """)
    ...
    **Note**: A higher detection rate (recall) means the model identifies more bankruptcies, but this often comes
    at the cost of more false alarms. The Altman Z-Score and Decision Tree have the highest bankruptcy detection rates but also higher
        ...
        if missing_cols:
            ...
            if not zscore_df.empty:
                ...
                if 'Actual Status' in zscore_df.columns:
                    ...
                    # Create comparison DataFrame with all models including Altman Z-score
                    comparison_data = {
                    ...
                    with col1:
                        ...
                        st.markdown("### Visual Representation")
                        st.write(f"**True Negative**: {10839:,} instances ({100 * 10839 / (10839 + 1156):.1f}% of actual alive)")
                        st.write(f"**False Positive**: {1156:,} instances ({100 * 1156 / (10839 + 1156):.1f}% of actual alive)")
                        st.write(f"**False Negative**: {203:,} instances ({100 * 203 / (203 + 84):.1f}% of actual bankrupt)")
                        st.write(f"**True Positive**: {84:,} instances ({100 * 84 / (203 + 84):.1f}% of actual bankrupt)")
                    
                    with col2:
                        st.markdown("### Z-Score Metrics")
                        # Using the values from the notebook
                        st.write(f"**True Negatives (TN)**: {10839:,}")
                        st.write(f"**False Positives (FP)**: {1156:,}")
                        st.write(f"**False Negatives (FN)**: {203:,}")
                        st.write(f"**True Positives (TP)**: {84:,}")
                        st.write("")
                        st.write(f"**Accuracy**: {0.8894:.4f}")
                        st.write(f"**Precision**: {0.0677:.4f}")
                        st.write(f"**Recall**: {0.2927:.4f}")
                        st.write(f"**F1 Score**: {0.1100:.4f}")
